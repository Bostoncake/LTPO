"""
main_vl_dmlr_search.py – Evaluate LTPO candidate-search variant on VL benchmarks
with the DMLR-compatible pipeline.

Difference from main_vl_dmlr.py:
- Imports `generate_vl` from `ltpo_vl_dmlr_search` (no mean update, picks
  the best sampled candidate A_i = H_0 + epsilon_i).
- Output-dir tag uses "-search" suffix instead of "-dmlr".

Everything else (answer extraction, LLM verification, args, main loop) is
unchanged from main_vl_dmlr.py.
"""
...
from ltpo_vl_dmlr_search import generate_vl
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate LTPO candidate-search variant on MLLM benchmarks "
                    "(DMLR-compatible pipeline, no mean update)"
    )
    ...
    # Candidate-search hyperparameters (lr kept for arg-compat; unused).
    parser.add_argument("--num_thought_tokens", type=int, default=2)
    parser.add_argument("--sigma", type=float, default=25.0)
    parser.add_argument("--sigma_decay", type=float, default=1.0,
                        help="1.0 = constant noise around H_0 (recommended for pure search)")
    parser.add_argument("--lr", type=float, default=0.0,
                        help="Unused in search mode; kept for argument compatibility")
    parser.add_argument("--max_num_steps", type=int, default=15,
                        help="Number of sampled candidates B")
...
def main(args):
    ...
    if args.eval_baseline:
        ...
        output_dir = (
            f"{args.output_dir}/{model_name}-{data_name}"
            f"-tokens{args.num_thought_tokens}"
