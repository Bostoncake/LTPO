"""
LTPO generate for VL — DMLR-compatible prompts + CONTRASTIVE reward + LOOK/THINK.

Same contrastive reward as ltpo_vl_dmlr_contrastive.py, plus a minimal
per-step LOOK/THINK control:

    delta = r1 - r2
    if delta < look_threshold: mode = 'look'   (small visual injection only)
    else:                      mode = 'think'  (original contrastive update)

LOOK does NOT replace the latent token; it just adds a small fraction
(`look_alpha`) of a mean-pooled visual summary onto it. THINK keeps the
existing contrastive LTPO update unchanged. Best-latent selection is
identical to the contrastive baseline (driven by `best_reward`).
"""
...
def build_inputs_vl_contrastive(
    ...
    data_name: str = '',
    model_name: str = '',
    match_baseline_prompt: bool = False,
):
    """Same as ltpo_vl_dmlr.build_inputs_vl but also returns an image-token mask.

    When ``match_baseline_prompt`` is True, the system + user content sent to the
    model is identical to the eval-baseline path (system=SYSTEM_PROMPT,
    user=question). The latent thought tokens are appended *after* the assistant
    generation prompt so LTPO can still locate and optimise them — they steer
    generation as already-emitted assistant embeddings rather than being
    advertised to the model inside the user turn.
    """
    ...
    if match_baseline_prompt:
        user_text = prompt
        append_thought_tokens = True
    else:
    ...
    if image is not None:
        if 'qwen' in model_name.lower():
            ...
            if append_thought_tokens:
                # Trailing '\n' gives get_confidence a valid position at
                # thought_idx[1] (it reads probs[thought_idx[1]], one past
                # the last thought token).
                text = text + latent_thought_tokens + '\n'
...
def generate_vl(
    ...
    enable_lookthink: bool = True,
    look_threshold: float = 0.0,
    look_alpha: float = 0.1,
    match_baseline_prompt: bool = False,
    **kwargs,
):
    ...
    inputs, thought_idx, image_mask = build_inputs_vl_contrastive(
        ...
        match_baseline_prompt=match_baseline_prompt,
    )
    ...
    step_rewards = {'opt_key': opt_key, 'best_key': best_key, 'opt': [], 'best': [],
                    'modes': [], 'look_count': 0, 'think_count': 0}

    # Pre-compute a single visual summary for LOOK: mean-pool the (already-merged)
    # visual embeddings at image-token positions. Detached so LOOK is a plain add.
    if enable_lookthink and image_mask is not None and image_mask.any():
        with torch.no_grad():
            visual_summary = inputs_embeds[0][image_mask[0]].mean(dim=0).detach()
    else:
        visual_summary = None
    ...
    for i in range(max_rl_steps):
        ...
        # LOOK / THINK gate: small contrastive gap -> inject visual; else keep
        # the original contrastive LTPO update.
        delta = float(r1) - float(r2)
        use_look = (
            enable_lookthink and visual_summary is not None and delta < look_threshold
        )
        if use_look:
            # LOOK: light visual re-injection only — no RL/grad update this step.
            mode = 'look'
            step_rewards['look_count'] += 1
            inject = look_alpha * visual_summary  # broadcast over thought tokens
            if use_auto_grad:
                with torch.no_grad():
                    thought_hidden_states.data.add_(inject)
            else:
                thought_hidden_states = thought_hidden_states + inject
        else:
            # THINK: unchanged contrastive LTPO update.
            mode = 'think'
            step_rewards['think_count'] += 1
            if use_auto_grad:
        ...
        step_rewards['modes'].append(mode)
        sigma *= sigma_decay

        if verbose:
            logger.info(
                f'>>> Step {i}  mode={mode}  delta={delta:.4f}  '
                f'opt[{opt_key}]={float(opt_reward_tensor):.4f}  '
