"""
LTPO generate for Vision-Language Models — DMLR-compatible, v2 simplified prompts.

Changes from ltpo_vl_dmlr.py:
- SYSTEM_PROMPT reduced to a single concise sentence (closer to baseline).
- input_content drastically simplified: no verbose "INTERNAL THINKING SPACE"
  paragraph, no multi-step format instructions.  Just the question, a short
  \boxed{} reminder, and the thought tokens.
- Everything else (RL loop, visual-token merging, generate_vl signature) is
  identical to ltpo_vl_dmlr.py.
"""
...
def build_inputs_vl(
    ...
    """
    Construct multimodal inputs for LTPO — v2 minimal prompt.

    Compared to ltpo_vl_dmlr.build_inputs_vl the prompt is much shorter:
    just the question followed by the thought tokens.  The system prompt
    already carries the \\boxed{} instruction.
    """
    ...
    # v2: keep it short — question + thought tokens, nothing else.
    input_content = (
        f'{prompt}\n\n'
