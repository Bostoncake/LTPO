"""
main_vl_workspace_contrastive.py – Evaluate LTPO + Contrastive Visual Reward.

Extends main_vl_workspace_fixed.py with a contrastive reward signal:

    reward = confidence(with_visual) - confidence(without_visual)

Each optimisation step does two forward passes to produce this contrastive
reward, encouraging latent tokens to actively leverage visual information.

New CLI arg compared to main_vl_workspace_fixed.py:
  --contrastive_visual_reward   enable the contrastive reward (default: off)
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate LTPO + Contrastive Visual Reward on MLLM benchmarks"
    )
    # Dataset / paths
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "reward = conf(with_visual) - conf(without_visual). "
...
def main(args):
    ...
    if args.use_workspace:
        ...
        contrastive_str = " + CONTRASTIVE" if args.contrastive_visual_reward else ""
        print(
    ...
    contrastive_suffix = "-contrastive" if args.contrastive_visual_reward else ""
    ...
    for i in tqdm(range(start_data_idx, end_data_idx)):
        ...
        if args.eval_baseline:
            ...
            # ---- LTPO optimised generation with contrastive visual reward ----
