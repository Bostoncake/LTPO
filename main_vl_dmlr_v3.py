"""
main_vl_dmlr_v3.py – Evaluate LTPO on Vision-Language benchmarks with a
DMLR-compatible pipeline — v3 dataset-specific minimal prompts.

v3 prompt changes (see ltpo_vl_dmlr_v3.py for full rationale):
  ...
  - User content in LTPO path: raw question + thought tokens only
    (no "PROBLEM:", no "INTERNAL THINKING SPACE" text).
  - Baseline path also uses get_system_prompt for consistency.

Everything else (model loading, RL loop, answer extraction, verification)
is identical to main_vl_dmlr.py / main_vl_dmlr_v2.py.

Usage:
    python main_vl_dmlr_v3.py \\
        --dataset scienceqa \\
        --data_root mllm_data \\
...
from ltpo_vl_dmlr_v3 import generate_vl, get_system_prompt
...
def main(args):
    ...
    # v3: dataset-specific system prompt
    ...
    for i in tqdm(range(start_data_idx, end_data_idx)):
        ...
        if args.eval_baseline:
            ...
            # ---- LTPO optimised generation (v3 prompts) ----
