...
Identical to ltpo_vl_dmlr_direct_boxed.py except for one fix carried
through every forward/generate call site: ``image_grid_thw`` is kept
alongside the pre-merged ``inputs_embeds`` and forwarded to every
``model(...)`` / ``model.generate(...)`` call.  Without this, Qwen2.5-VL
falls back to 1D position_ids (mRoPE is disabled) whenever inputs are
fed as ``inputs_embeds`` — which is the case for the entire LTPO path.
"""
...
def build_inputs_vl(
    ...
    # Keep image_grid_thw so Qwen2.5-VL can compute proper mRoPE
    # position_ids when generating from inputs_embeds (no pixel_values).
    if 'image_grid_thw' in inputs and inputs['image_grid_thw'] is not None:
        clean_inputs['image_grid_thw'] = inputs['image_grid_thw']
    ...
    if thought_init_from_hidden and thought_idx[0] > 0:
        with torch.no_grad():
            ...
            if 'image_grid_thw' in inputs:
                fwd_kwargs['image_grid_thw'] = inputs['image_grid_thw']
            fwd = model(**fwd_kwargs)
