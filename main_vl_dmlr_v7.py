"""
main_vl_dmlr_v6.py – Evaluate LTPO on Vision-Language benchmarks with a
DMLR-compatible pipeline — v6 cherry-picked best prompts per dataset.

v6 prompt design (see ltpo_vl_dmlr_v6.py for full rationale):
  - Baseline mode (--eval_baseline): ALL datasets use the unified v2
    system_prompt ("Please reason step by step, and MUST put your final
    answer within \\boxed{}.").  No thought tokens, no bridge text.
  - LTPO mode: each dataset gets a cherry-picked system_prompt AND a
    dataset-specific prompt_instruction (user-content bridge text) from
    whichever version (v2/v3/v4) performed best:
      * MathVista     → v4 (strategy-hint system prompt + short bridge)
      * MathVision    → v3 (plain system prompt + no bridge)
      * MM-Math       → v2 (MUST system prompt + long bridge)
      * HallusionBench→ v4 (strategy-hint system prompt + short bridge)
      * MMVP          → v2 (MUST system prompt + long bridge)
      * MMStar        → v4 (strategy-hint system prompt + short bridge)
      * ScienceQA     → v4 (strategy-hint system prompt + short bridge)

Everything else (model loading, RL loop, answer extraction, verification)
is identical to main_vl_dmlr_v4.py.

Usage:
    python main_vl_dmlr_v6.py \\
        --dataset scienceqa \\
        --data_root mllm_data \\
...
def main(args):
    ...
    for i in tqdm(range(start_data_idx, end_data_idx)):
        ...
        if args.eval_baseline:
            # ---- Baseline: standard VL inference (v2 unified system_prompt) ----
            ...
            # ---- LTPO optimised generation (v6 prompts) ----
