...
def build_inputs_vl(
    ...
    if use_baseline_prompt:
        ...
        if no_boxed_prefix:
            # Trace-generation variant: keep the latent thought tokens
            # in the assistant turn (so LTPO injection still has a slot),
            # but DROP the forced "\\boxed{" prefix so the model is free
            # to produce its real reasoning path.
        ...
        # Trace-generation variant: latent thought tokens are already in
        # the user content; drop the forced "\\boxed{" prefix so the
        # assistant turn starts empty and the model can write a real
        # chain-of-thought.
        assistant_suffix = "" if no_boxed_prefix else ASSISTANT_BOXED_PREFIX
...
def _pool_top_p_visual_tokens(
    ...
    When ``return_vis_info=True`` the returned tuple is extended with a
    visualization dict carrying the per-image-token attention weights and
    the selected indices/weights so callers can draw the look event on
    the original image without recomputing the pooling.
    """
    if (
    ...
    selected_weights_norm = selected_weights / selected_weights.sum().clamp_min(1e-12)
    selected_embeds = visual_inputs_embeds[0, img_pos[keep_order]]
    pooled = torch.sum(
        selected_embeds * selected_weights_norm.to(selected_embeds.dtype).unsqueeze(-1),
        dim=0,
    )
    mass = float(weights[keep_order].sum())
    if return_vis_info:
        vis_info = {
            "weights_full": weights.detach().cpu(),                 # (n_img,)
            "keep_order": keep_order.detach().cpu(),                # (k,)
            "selected_weights_pre_renorm": selected_weights.detach().cpu(),
            "selected_weights_norm": selected_weights_norm.detach().cpu(),
            "top_p": p,
            "mass": mass,
            "n_image_tokens": int(scores.numel()),
        }
        return pooled.detach(), int(keep_n), mass, vis_info
...
def generate_vl(
    ...
    reward_on_latent_tokens: bool = False,
    vis_lookthink_callback=None,
    trace_gen_callback=None,
    trace_gen_max_new_tokens: int = 512,
    **kwargs,
):
    """
    ...
    # ---- Per-step trace-generation inputs (no "\boxed{" forced prefix) ----
    # Built once per sample when a trace_gen_callback is provided. Used to
    # generate a free-form reasoning trace after every optimisation step
    # so callers can observe the optimisation effect on the actual
    # generated thinking path (not the boxed answer).
    trace_inputs = None
    trace_thought_idx = None
    if trace_gen_callback is not None:
        try:
            trace_inputs, trace_thought_idx, _ = build_inputs_vl(
                processor=processor,
                model=model,
                image=image,
                num_thought_tokens=num_thought_tokens,
                ...
                use_baseline_prompt=use_baseline_prompt,
                use_inputs_embeds=use_inputs_embeds,
                no_boxed_prefix=True,
            )
        except Exception as _terr:
            logger.warning(
                f'failed to build trace_inputs; trace generation disabled: '
                f'{type(_terr).__name__}: {_terr}'
            )
            trace_inputs = None
            trace_thought_idx = None
    ...
    for i in range(max_rl_steps):
                    ...
                    if reward_type == "entropy_diff":
                        ...
                        if gate_open:
                            pooled_info = _pool_top_p_visual_tokens(
                                ...
                                return_vis_info=vis_lookthink_callback is not None,
                            )
                            if pooled_info is not None:
                                if vis_lookthink_callback is not None and len(pooled_info) == 4:
                                    (
                                    ...
                                    try:
                                        ...
                                        logger.warning(
                                            f'vis_lookthink_callback failed at step {i}: '
                                            f'{type(_vis_err).__name__}: {_vis_err}'
                                        )
                                else:
                                    (
                                        lookthink_visual_update,
        ...
        # ---- Per-step trace generation (free-form, no "\boxed{" prefix) ----
        # Runs after the parameter update of step i. Uses the current
        # thought_hidden_states (NOT the best-tracked snapshot) so the
        # caller can observe what the optimisation has actually changed
        # in the model's free generation. Independent from the boxed-
        # answer logits used by best-step selection.
        if trace_gen_callback is not None and trace_inputs is not None:
            try:
                with torch.no_grad():
                    cur_thought_for_trace = thought_hidden_states.detach()
                    if use_inputs_embeds:
                        trace_emb = trace_inputs['inputs_embeds'].clone()
                        if trace_thought_idx[1] > trace_thought_idx[0]:
                            trace_emb[
                                0,
                                trace_thought_idx[0]:trace_thought_idx[1],
                            ] = cur_thought_for_trace.to(
                                dtype=trace_emb.dtype,
                                device=trace_emb.device,
                            )
                        trace_gen_inputs = dict(trace_inputs)
                        trace_gen_inputs['inputs_embeds'] = trace_emb
                        trace_outputs = model.generate(
                            **trace_gen_inputs,
                            max_new_tokens=trace_gen_max_new_tokens,
                            do_sample=False,
                            temperature=0.0,
                            top_p=None,
                            num_beams=1,
                        )
                        trace_decoded = tokenizer.decode(
                            trace_outputs[0], skip_special_tokens=True
                        )
                    else:
                        trace_injector = ThoughtEmbedInjector(
                            trace_thought_idx[0],
                            trace_thought_idx[1],
                            cur_thought_for_trace,
                        )
                        trace_handle = model.language_model.register_forward_pre_hook(
                            trace_injector, with_kwargs=True
                        )
                        try:
                            trace_outputs = model.generate(
                                **trace_inputs,
                            ...
                            trace_handle.remove()
                        trace_input_length = trace_inputs['input_ids'].shape[1]
                        trace_new_tokens = trace_outputs[0, trace_input_length:]
                        trace_decoded = tokenizer.decode(
                            trace_new_tokens, skip_special_tokens=True
                        )
                trace_meta = {
                    "step_idx": i,
                    ...
                    "lookthink_mode": lookthink_mode,
                    "lookthink_selected_count": int(lookthink_selected_count),
                    "lookthink_selected_mass": float(lookthink_selected_mass),
                    "sigma_after_decay": float(sigma),
                }
                trace_gen_callback(
                    step_idx=i,
                    decoded=trace_decoded,
                    meta=trace_meta,
                )
            except Exception as _terr:
                logger.warning(
                    f'trace_gen_callback failed at step {i}: '
                    f'{type(_terr).__name__}: {_terr}'
                )
