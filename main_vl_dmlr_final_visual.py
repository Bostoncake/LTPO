"""
main_vl_dmlr_final_visual.py — evaluate the FINAL + DMLR-visual variant.

Drops every alternative branch from main_vl_dmlr_final.py. The only
supported configuration is endoftext-init + confidence reward on the
latent thought tokens with DMLR's per-step visual-token injection
(see ltpo_vl_dmlr_final_visual.generate_vl).
"""
...
from ltpo_vl_dmlr_final_visual import generate_vl
...
# ---------------------------------------------------------------------------
# Answer extraction — same as main_vl_dmlr_final.
# ---------------------------------------------------------------------------
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate FINAL + DMLR-visual variant on MLLM benchmarks"
    )
    ...
    # LTPO knobs (same defaults as final).
    ...
    # DMLR visual-injection knobs (defaults from DMLR/script/run.sh).
    parser.add_argument("--num_selected_patches", type=int, default=16)
    parser.add_argument("--initial_patch_count", type=int, default=1)
    parser.add_argument("--patch_increment", type=int, default=1)
...
def main(args):
    ...
    output_dir = (
        ...
        f"-patches{args.num_selected_patches}-init{args.initial_patch_count}"
        f"-inc{args.patch_increment}-stride{args.visual_insert_stride}"
        f"-start{args.visual_injection_start_step}-itv{args.visual_injection_interval}"
        f"-conf-boxed-baseprompt-visual" + output_suffix
    )
