"""
main_vl_workspace.py – Evaluate LTPO + Visual Workspace on VL benchmarks.

Extends main_vl_dmlr.py with three new CLI args that control the workspace:
  --use_workspace          enable visual workspace (default: off)
  --num_workspace_slots K  workspace capacity (default: 8)
  --num_route_slots r      slots injected per optimisation step (default: 2)
  --workspace_inject_mode  "add" (default) | "prepend"

All other args and the evaluation / verification logic are identical to
main_vl_dmlr.py.  The base DMLR / LTPO paths are fully preserved:
  - Without --use_workspace this script behaves like main_vl_dmlr.py.
  - main_vl_dmlr.py itself is never modified.
"""
...
from ltpo_vl_workspace import generate_vl_workspace
from visual_workspace import WorkspaceConfig
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate LTPO + Visual Workspace on MLLM benchmarks"
    )
    # Dataset / paths
    ...
    # ---- Visual Workspace (new args) ----
    parser.add_argument(
        "--use_workspace", action="store_true",
        help="Enable visual workspace + routing (default: off)",
    )
    parser.add_argument(
        "--num_workspace_slots", type=int, default=8,
        help="K: workspace capacity (number of slots built per sample)",
    )
    parser.add_argument(
...
def main(args):
    ...
    if args.use_workspace:
        print(
            f"[Workspace] enabled  K={ws_config.num_workspace_slots}  "
            f"r={ws_config.num_route_slots}  mode={ws_config.workspace_inject_mode}"
        )
    ...
    ws_suffix = (
        f"-ws{ws_config.num_workspace_slots}r{ws_config.num_route_slots}"
    ...
    for i in tqdm(range(start_data_idx, end_data_idx)):
        ...
        if args.eval_baseline:
            ...
            # ---- LTPO optimised generation with optional workspace ----
            output, best_reward, best_reward_step, stop_reason = generate_vl_workspace(
                processor=processor,
                model=model,
