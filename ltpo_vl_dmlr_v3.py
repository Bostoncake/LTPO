"""
LTPO generate for Vision-Language Models — DMLR-compatible, v3 prompts.

v3 prompt design rationale
==========================
v1 added verbose instructions ("PROBLEM:", "IMPORTANT:", "INTERNAL THINKING
SPACE" paragraph, multi-choice detection block) on top of DMLR's <think>/
<answer> system prompt.  v2 stripped the system prompt down and removed the
"PROBLEM:" prefix but kept the "INTERNAL THINKING SPACE" paragraph.

Both v1 and v2 performed worse than the baseline.  The likely reason is that
the extra text confused the model:
  - The data prompts already carry their own \boxed{} / choice instructions.
  - Describing the thought tokens in natural language added noise.

v3 changes:
  - **Dataset-specific SYSTEM_PROMPT** via get_system_prompt(data_name):
      * hallusion → Yes/No + \boxed{}
      * MCQ datasets (mmvp, mmstar, scienceqa, math_vision with choices) →
        option letter + \boxed{}
      * math datasets (mm_math, math_vista, math_vision open-ended) →
        step-by-step + \boxed{}
    Each is a single concise sentence, close to what the baseline uses.
  - **input_content**: just `{question}\n{thought_tokens}`.
    NO "PROBLEM:" prefix, NO "INTERNAL THINKING SPACE" description, NO extra
    instructions.  The thought tokens are silently appended so the model
    treats them as context rather than something it must reason about.
...
_SYSTEM_PROMPT_MCQ = (
    "Please reason step by step, and put your final answer (the option letter) within \\boxed{}."
)

_SYSTEM_PROMPT_YESNO = (
    "Please reason step by step, and put your final answer (Yes or No) within \\boxed{}."
)

# Fallback — same as baseline
_SYSTEM_PROMPT_DEFAULT = _SYSTEM_PROMPT_MATH


def get_system_prompt(data_name: str) -> str:
    """Return a concise, dataset-appropriate system prompt."""
    dn = data_name.lower() if data_name else ""
    if "hallusion" in dn:
        return _SYSTEM_PROMPT_YESNO
    if any(k in dn for k in ("mmvp", "mmstar", "scienceqa")):
        return _SYSTEM_PROMPT_MCQ
    # mm_math, math_vista, math_vision → general math
    return _SYSTEM_PROMPT_MATH


# For backward compat (main_vl_dmlr_v3 imports this for baseline path)
...
# ---------------------------------------------------------------------------
# build_inputs_vl  — v3 minimal, dataset-aware prompt
# ---------------------------------------------------------------------------
...
def build_inputs_vl(
    ...
    """
    Construct multimodal inputs for LTPO — v3 minimal prompt.

    The system prompt is dataset-specific (get_system_prompt).
    The user content is just the raw question + thought tokens — no extra
    instructions, no "PROBLEM:" prefix, no "INTERNAL THINKING SPACE" text.
    """
    ...
    # v3: question + thought tokens, nothing else
