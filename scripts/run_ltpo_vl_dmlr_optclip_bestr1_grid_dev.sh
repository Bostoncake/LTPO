#!/bin/bash
# run_ltpo_vl_dmlr_optclip_bestr1_grid_dev.sh
# Hyperparameter sweep for the DECOUPLED (opt=clip, best=r1) reward
# combination on all 7 dev sets.
#
# Reward setup:
#   opt_reward  = clip   (optimise on r1 + lambda * max(r1 - r2, 0))
#   best_reward = r1     (pick best latent by r1 alone)
#
# Per-dataset gap_lambda is fixed to the best gapbonus value found by
#   python scripts/find_best_grid.py \
#       output/ltpo_dmlr_contrastive_grid_dev/0425_contrast_clipping_qwen2.5_7B
#   (col "gapbonus" in the top-1 row of each dataset).
#
# Sweeps: num_thought_tokens x max_num_steps x sigma x lr
# Fixed:  sigma_decay=0.95, top_k=10
#
# Usage:
#   N_GPUS=8 bash scripts/run_ltpo_vl_dmlr_optclip_bestr1_grid_dev.sh
...
root_output=./output/ltpo_dmlr_contrastive_grid_dev/0426_contrast_optclip_bestr1
mkdir -p "${root_output}"
...
echo "========================================================"
echo "LTPO-DMLR DECOUPLED (opt=clip, best=r1) Grid Search on dev sets"
...
while [ $job_idx -lt $total ]; do
    for ((g=0; g<N_GPUS; g++)); do
        ...
        if [ "$pid" -eq -1 ] || ! kill -0 "$pid" 2>/dev/null; then
            ...
            tag="tokens${tokens}_steps${steps}_sigma${sigma}_decay${SIGMA_DECAY}_lr${lr}_topk${TOP_K}_optclip_bestr1"
