#!/bin/bash
# run_ltpo_vl_dmlr_final_lookthink_param_search.sh
#
# LOOK/THINK hyperparam sweep on the FINAL variant (main_vl_dmlr_final.py),
# based on run_ltpo_vl_dmlr_final_compound_best_selection_param_search.sh.
#
# Fixed settings:
...
#   lookthink_threshold in {0, -0.1}
#
# Usage:
#   bash scripts/run_ltpo_vl_dmlr_final_lookthink_param_search.sh
#
...
LOOKTHINK_THRESHOLD_LIST=(0 -0.1)

root_output=./output/ltpo_dmlr_final/0517_param_search_perds_init_lookthink
mkdir -p "${root_output}"
...
for lr in "${LR_LIST[@]}"; do
    for dataset in "${DATASETS[@]}"; do
        for top_p in "${LOOKTHINK_TOP_P_LIST[@]}"; do
            for threshold in "${LOOKTHINK_THRESHOLD_LIST[@]}"; do
                jobs+=("${lr} ${dataset} ${top_p} ${threshold}")
            done
        done
    done
done
...
while [ $job_idx -lt $total ]; do
    for ((g=0; g<N_GPUS; g++)); do
        ...
        if [ "$pid" -eq -1 ] || ! kill -0 "$pid" 2>/dev/null; then
            read -r lr dataset top_p threshold <<< "${jobs[$job_idx]}"
            ...
            tag="lookthink_reward${REWARD_TYPE}_best${BEST_SELECTION}_steps${STEPS}_sigma${SIGMA}_decay${SIGMA_DECAY}_lr${lr}_topk${TOP_K}_topp${top_p}_thr${threshold}"
