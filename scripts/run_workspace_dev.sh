#!/bin/bash
# run_workspace_dev.sh — LTPO + Visual Workspace (original) on 300-sample dev splits.
#
...
output=./output/dev/workspace_${INJECT_MODE}_K${NUM_WORKSPACE_SLOTS}_r${NUM_ROUTE_SLOTS}
mkdir -p "${output}"
