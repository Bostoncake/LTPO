...
# for Qwen2.5-VL-3B. Mirrors the qwen3vl8b sweep script but uses the
# per-dataset best (num_thought_tokens, init_mode) discovered for the 3B
# model, where several datasets prefer mean-embedding initialisation.
#
# Per-dataset best init (qwen2.5-VL-3B):
#   MathVista  - endoftext init, 1 token
#   MathVision - endoftext init, 1 token
#   MM-Math    - mean init,      3 tokens
#   Hallusion  - endoftext init, 1 token
#   MMVP       - endoftext init, 5 tokens
#   MMStar     - mean init,      5 tokens
#   ScienceQA  - mean init,      5 tokens
#
# Fixed settings:
...
#
# Usage:
#   bash scripts/run_ltpo_vl_dmlr_final_lookthink_stag_param_search_qwen25vl3b.sh
#
...
root_output=./output/ltpo_dmlr_final/0521_param_search_perds_init_lookthink_stag_qwen25vl3b
mkdir -p "${root_output}"
...
echo "========================================================"
echo "LTPO-DMLR FINAL LOOK/THINK (stagnation gate) param-search [qwen2.5-VL-3B]"
