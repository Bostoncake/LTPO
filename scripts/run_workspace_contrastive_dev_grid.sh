#!/bin/bash
# run_workspace_contrastive_dev_grid.sh
# Workspace-Contrastive hyperparameter grid search on all 7 dev sets.
#
# Contrastive method: at each ES step the same noised thought is evaluated
# under TWO forward passes — without visual evidence (r1) and with visual
# evidence (r2) — and the ES reward is r = r2 - r1. Only noise directions
# that are verified as helpful by the injected visual information are
# reinforced.
#
...
#
# Workspace grid (mirrors run_workspace_fixed_dev_grid.sh):
#   K (num_workspace_slots) ∈ {8, 16}
#   r (num_route_slots)     ∈ {2, 4}
#   inject_mode             ∈ {prepend, add}
#   route_mode              ∈ {per_token}
#
# Total: 2×2×2×1 = 8 configs × 7 datasets = 56 jobs.
# Est. wall time (8 GPUs): ≈ 2× the fixed grid because every ES step runs
# two forward passes.
#
# Usage:
#   N_GPUS=8 bash scripts/run_workspace_contrastive_dev_grid.sh
#
...
root_output=./output/workspace_contrastive_dev_grid
mkdir -p "${root_output}"
...
echo "════════════════════════════════════════════════════════════════"
echo "Workspace Contrastive Grid Search on dev sets  (reward = r2 - r1)"
...
echo "    Est. wall time (2× fixed-grid): $(( (total + N_GPUS - 1) / N_GPUS * 10 )) min"
echo "════════════════════════════════════════════════════════════════"
